 ...
 * FactoryCore or ApartmentCore can bind to it <em>reflectively</em> &mdash;
 * giving a genuine soft dependency with no shared build artifact and no
 * load-order coupling beyond a {@code softdepend} entry in the consumer's
 * {@code plugin.yml}.
 *
...
public final class NationCoreAPI {
    ...
    private static ApartmentTaxIntegration apartmentIntegration() {
        NationCore plugin = NationCore.getInstance();
        return plugin != null ? plugin.getApartmentTaxIntegration() : null;
    }
    ...
    /**
     * @return {@code true} when the apartment-tax integration is switched on in
     *         NationCore's {@code config.yml} and the plugin is initialised.
     */
    public static boolean isApartmentTaxIntegrationEnabled() {
        try {
            ApartmentTaxIntegration i = apartmentIntegration();
            return i != null && i.isEnabled();
        } catch (Throwable t) {
            return false;
        }
    ...
    /**
     * Routes an apartment tax payment into the treasury of the nation that owns
     * the land at {@code loc}.
     *
     ...
     * @return the amount actually banked into a nation treasury; {@code 0.0} when
     *         nothing was routed. Never throws.
     * @see ApartmentTaxIntegration#depositApartmentTax(Location, double, UUID, String)
     */
    public static double depositApartmentTax(Location loc, double amount, UUID payer, String sourceLabel) {
        try {
            ApartmentTaxIntegration i = apartmentIntegration();
            return i != null ? i.depositApartmentTax(loc, amount, payer, sourceLabel) : 0.0;
        } catch (Throwable t) {
            return 0.0;
        }
    }
