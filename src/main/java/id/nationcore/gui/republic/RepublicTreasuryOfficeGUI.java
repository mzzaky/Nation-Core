...
public class RepublicTreasuryOfficeGUI extends NationMenuBase {

    public static final String TITLE = "§9§lMINISTER OF TREASURY OFFICE";

    public RepublicTreasuryOfficeGUI(NationCore plugin) {
        super(plugin);
    }
    ...
    public void open(Player player, Nation nation) {
        ...
        // Security check: only President, Treasury Minister, and Admins can open
        boolean isPresident = gov.hasPresident() && gov.getPresidentUUID().equals(player.getUniqueId());
        boolean isTreasuryMinister = gov.getCabinetMember(Government.CabinetPosition.TREASURY) != null && gov.getCabinetMember(Government.CabinetPosition.TREASURY).equals(player.getUniqueId());
        boolean isAdmin = player.hasPermission("nation.admin");
        if (!isPresident && !isTreasuryMinister && !isAdmin) {
