...
public class RepublicHealthOfficeGUI extends NationMenuBase {

    public static final String TITLE = "§9§lMINISTER OF HEALTH OFFICE";

    public RepublicHealthOfficeGUI(NationCore plugin) {
        super(plugin);
    }
    ...
    public void open(Player player, Nation nation) {
        ...
        // Security check: only President, Health Minister, and Admins can open
        boolean isPresident = gov.hasPresident() && gov.getPresidentUUID().equals(player.getUniqueId());
        boolean isHealthMinister = gov.getCabinetMember(Government.CabinetPosition.HEALTH) != null && gov.getCabinetMember(Government.CabinetPosition.HEALTH).equals(player.getUniqueId());
        boolean isAdmin = player.hasPermission("nation.admin");
        if (!isPresident && !isHealthMinister && !isAdmin) {
        ...
        inv.setItem(20, buildDecisionItem(nation, CabinetDecision.DecisionType.QUARANTINE_PROTOCOL, player));
        inv.setItem(22, buildDecisionItem(nation, CabinetDecision.DecisionType.FIELD_MEDICINE, player));
        inv.setItem(24, buildDecisionItem(nation, CabinetDecision.DecisionType.VACCINATION_DRIVE, player));
        inv.setItem(30, buildDecisionItem(nation, CabinetDecision.DecisionType.EMERGENCY_RATIONS, player));
        inv.setItem(32, buildDecisionItem(nation, CabinetDecision.DecisionType.PLAGUE, player));
