...
public class RepublicHealthOfficeGUI extends NationMenuBase {
    ...
    public void open(Player player, Nation nation) {
        ...
        boolean isMinister = gov.getCabinetMember(Government.CabinetPosition.HEALTH) != null
                && gov.getCabinetMember(Government.CabinetPosition.HEALTH).equals(player.getUniqueId());
        ...
        for (int i = 0; i < orders.size() && i < eoSlots.length; i++) {
            inv.setItem(eoSlots[i], buildExecutiveOrderCard(nation, orders.get(i), canIssue, "Minister of Health", "&b"));
        }
