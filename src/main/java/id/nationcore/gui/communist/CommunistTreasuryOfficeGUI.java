...
public class CommunistTreasuryOfficeGUI extends NationMenuBase {
    ...
    public CommunistTreasuryOfficeGUI(NationCore plugin) {
        super(plugin);
    }
    ...
    public void open(Player player, Nation nation) {
        ...
        // Security check: only Secretary General, Treasury Minister, and Admins can open
        boolean isSekjen = gov.hasSecretaryGeneral() && gov.getSecretaryGeneralUUID().equals(player.getUniqueId());
        boolean isTreasuryMinister = gov.getPolitburoMember(CommunistGovernment.PolitburoPosition.TREASURY) != null
        ...
        if (!isSekjen && !isTreasuryMinister && !isAdmin) {
    ...
    private ItemStack buildDecisionItem(Nation nation, CommunistDecisionType type, Player viewer) {
        ...
        if (active && gov != null) {
            if (type == CommunistDecisionType.TRE_DISTRIBUTION_PROGRAM) {
                ...
                lore.add("&7Phases Left: &a" + gov.getTaxIntensificationPhasesLeft());
            } else {
                long remaining = 0;
                if (type == CommunistDecisionType.TRE_MARKET_EVENT) {
                    remaining = gov.getMarketEventUntil() - System.currentTimeMillis();
                }
                if (remaining > 0) {
